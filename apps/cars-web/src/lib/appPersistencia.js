...
export function aplicarDadosCarregados(data, S) {
  ...
  S.setSnapshotsCarteira?.(data.snapshotsCarteira || []);
  ...
  // Memória permanente do Jarbas.
  if (S.setJarbasMemoria) S.setJarbasMemoria(data.jarbasMemoria || []);
...
export function aplicarSeeds(S) {
  ...
  if (S.setJarbasMemoria) S.setJarbasMemoria([]);
