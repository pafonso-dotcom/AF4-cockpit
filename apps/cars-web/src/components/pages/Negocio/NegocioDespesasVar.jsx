...
export default function NegocioDespesasVar({ despesas = [], setDespesas, categorias = [], contas = [], lojaAtiva, lojas = [], hidden }) {
  ...
  const save = () => {
    ...
    if (form.id && despesas.find(d => d.id === form.id)) {
      ...
      toast.success(`Despesa "${form.descricao}" criada.`);
    }
    setForm(null);
    setFormErrors({});
  };
  ...
  const excluir = async (d) => {
    const ok = await confirm({
      ...
      body: "A despesa variável do Negócio será removida.",
    ...
    <div className="fade-up py-8">
      <div style={{ marginBottom: 16, paddingBottom: 14, borderBottom: `1px solid ${T.border}`,
        ...
        <div style={{ flexShrink: 0 }}>
          ...
          <div style={{ color: T.muted, fontSize: 12, marginTop: 4, fontStyle: "italic" }}>
            Despesas avulsas do Negócio.
          </div>
        </div>
        <button className="btn-gold" style={{ padding: "7px 12px", fontSize: 11 }} onClick={novo}>
          <Plus size={13} className="inline mr-1.5" />Nova despesa
        </button>
      </div>

      {/* Total + filtro por mês */}
      <div style={{
        ...
        <select value={mesFiltro} onChange={e => setMesFiltro(e.target.value)}
                style={{ background: T.bgSoft, border: `1px solid ${T.border}`, color: T.ink,
      ...
      {filtradas.length === 0 ? (
        <div style={{ padding: 50, textAlign: "center", color: T.muted, fontStyle: "italic",
          ...
          Nenhuma despesa variável {mesFiltro ? `em ${mesLabel(mesFiltro)}` : "do Negócio ainda"}.
          {!mesFiltro && <> Comece com o botão <strong>Nova despesa</strong>.</>}
        </div>
      ) : (
      ...
      {form && (
        <Modal title={form.id ? "Editar despesa" : "Nova despesa variável"} onClose={() => setForm(null)}>
          <Field label="Descrição" required error={formErrors.descricao}>
            <input value={form.descricao} onChange={e => setForm({ ...form, descricao: e.target.value })} placeholder="Ex.: Material de escritório" />
          </Field>
