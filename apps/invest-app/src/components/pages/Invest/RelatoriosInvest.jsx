...
/* ============================================================
   MovInvestMes — seção recolhível "Movimentações do mês"
   (compras, vendas com resultado, proventos) + resumo, seletor
   de mês e export PDF. Autossuficiente.
   ============================================================ */
const MI_MESES = ["janeiro","fevereiro","março","abril","maio","junho","julho","agosto","setembro","outubro","novembro","dezembro"];
const miNomeMes = (iso) => { const [a,m] = (iso||"").split("-").map(Number); return `${MI_MESES[(m||1)-1]} ${a||""}`; };

function MovInvestMes({ transacoes = [], hidden = false }) {
  const hoje = new Date();
  const mesAtual = `${hoje.getFullYear()}-${String(hoje.getMonth()+1).padStart(2,"0")}`;
  const [mes, setMes] = useState(mesAtual);
  const [aberto, setAberto] = useState(() => { try { return localStorage.getItem("af4:secao:mov-invest-mes") === "1"; } catch { return false; } });
  const toggle = () => setAberto(v => { const n = !v; try { localStorage.setItem("af4:secao:mov-invest-mes", n ? "1" : "0"); } catch {} return n; });
  ...
  const vazio = totalMov === 0;
  const dia = (iso) => (iso||"").slice(8,10) + "/" + (iso||"").slice(5,7);

  const exportarPDF = () => {
    const brl = (v) => `R$ ${Math.round(Number(v)||0).toLocaleString("pt-BR")}`;
    const tab = (t, l) => l.length ? `<h2>${t}</h2><table>${l.join("")}</table>` : "";
    printHTML(`<!doctype html><html><head><meta charset="utf-8"><title>Movimentações · ${miNomeMes(mes)}</title>
<style>body{font-family:system-ui,sans-serif;color:#222;padding:24px}h1{font-size:20px;margin:0}h2{font-size:13px;margin:16px 0 5px;color:#555}table{width:100%;border-collapse:collapse;font-size:12px}td{padding:4px 6px;border-bottom:1px solid #eee}td.n{text-align:right}</style></head><body>
<h1>Movimentações de investimentos · ${miNomeMes(mes)}</h1>
${tab("Compras (aportes)", mov.compras.map(c=>`<tr><td><b>${c.ticker}</b> · ${dia(c.data)}</td><td class="n">${brl(c.valor)}</td></tr>`))}
${tab("Vendas", mov.vendas.map(v=>`<tr><td><b>${v.ticker}</b> · ${dia(v.data)}${v.resultado!=null?` · result. ${brl(v.resultado)}`:""}</td><td class="n">${brl(v.valor)}</td></tr>`))}
${tab("Proventos", mov.proventos.map(p=>`<tr><td><b>${p.ticker}</b> · ${p.tipo} · ${dia(p.data)}</td><td class="n">${brl(p.valor)}</td></tr>`))}
<h2>Resumo</h2><table><tr><td>Comprado</td><td class="n">${brl(mov.totalComprado)}</td></tr><tr><td>Vendido</td><td class="n">${brl(mov.totalVendido)}</td></tr><tr><td>Proventos</td><td class="n">${brl(mov.totalProventos)}</td></tr><tr><td>Resultado das vendas</td><td class="n">${brl(mov.resultadoVendas)}</td></tr></table>
</body></html>`);
  };
  ...
  const pill = { display: "inline-flex", alignItems: "center", background: T.bgSoft, border: `1px solid ${T.border}`, borderRadius: 8, padding: "2px 8px", fontSize: 11.5, fontWeight: 800, flexShrink: 0 };
  ...
  return (
    <div style={{ marginTop: 14, marginBottom: 10 }}>
      <div onClick={toggle} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderRadius: 12, background: T.bgSoft, border: `1px solid ${T.border}`, cursor: "pointer" }}>
        <span style={{ color: T.gold, transform: aberto ? "rotate(90deg)" : "none", transition: "transform .15s" }}>›</span>
        <span style={{ fontSize: 12.5, fontWeight: 600, color: T.ink, flex: 1 }}>
          Movimentações · <span style={{ textTransform: "capitalize", color: T.gold }}>{miNomeMes(mes)}</span>
          {totalMov > 0 && <span style={{ color: T.faint, fontWeight: 500 }}> · comprado {oculto(mov.totalComprado)} · proventos {oculto(mov.totalProventos)}</span>}
        </span>
        <span style={{ fontSize: 10.5, color: T.muted, background: T.border, borderRadius: 100, padding: "1px 8px" }}>{totalMov}</span>
      </div>
        ...
        <div style={{ marginTop: 10 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
              <button onClick={() => passo(-1)} style={navB}>‹</button>
              <span style={{ fontFamily: T.serif, fontSize: 15, fontWeight: 600, textTransform: "capitalize", minWidth: 120, textAlign: "center" }}>{miNomeMes(mes)}</span>
              <button onClick={() => passo(1)} style={navB}>›</button>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 12 }}>
            {[{ k: "Comprado", v: oculto(mov.totalComprado), a: T.green, c: T.ink },
              ...
              { k: "Result. vendas", v: (mov.resultadoVendas >= 0 ? "+ " : "− ") + oculto(Math.abs(mov.resultadoVendas)), a: mov.resultadoVendas >= 0 ? T.green : T.red, c: mov.resultadoVendas >= 0 ? T.green : T.red }].map((t, i) => (
              <div key={i} style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: "12px 13px", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ fontFamily: T.serif, fontSize: 18, fontWeight: 700, marginTop: 2, color: t.c }}>{t.v}</div>
              </div>
            ))}
          </div>
          ...
          {vazio ? (
            <div style={{ ...box, textAlign: "center", color: T.muted, fontStyle: "italic", fontSize: 13, padding: 20 }}>
              Nenhuma movimentação em {miNomeMes(mes)}. (Compras/vendas aparecem quando o aporte/venda é feito com uma conta selecionada.)
            </div>
          ) : (<>
            {mov.compras.length > 0 && (<div style={box}>{secH("🟢", T.green, "Compras (aportes)", mov.compras.length, mov.totalComprado)}
              {mov.compras.map((c) => linha(<><span style={pill}>{c.ticker}</span><span style={{ flex: 1, minWidth: 0, fontSize: 11, color: T.muted }}>{c.qtd > 0 ? `${fmtN(c.qtd, c.qtd < 1 ? 8 : 0)} × ${oculto(c.preco)} · ` : ""}{dia(c.data)}</span><span style={{ fontFamily: T.serif, fontWeight: 700 }}>{oculto(c.valor)}</span></>))}
            </div>)}
            {mov.vendas.length > 0 && (<div style={box}>{secH("🔴", T.red, "Vendas", mov.vendas.length, mov.totalVendido)}
              {mov.vendas.map((v) => linha(<><span style={pill}>{v.ticker}</span><span style={{ flex: 1, minWidth: 0, fontSize: 11, color: T.muted }}>{v.qtd > 0 ? `${fmtN(v.qtd, v.qtd < 1 ? 8 : 0)} × ${oculto(v.preco)} · ` : ""}{dia(v.data)}</span>{v.resultado != null && <span style={{ fontSize: 10.5, fontWeight: 700, padding: "2px 8px", borderRadius: 100, background: `${v.resultado >= 0 ? T.green : T.red}18`, color: v.resultado >= 0 ? T.green : T.red }}>{v.resultado >= 0 ? "+ " : "− "}{oculto(Math.abs(v.resultado))}</span>}<span style={{ fontFamily: T.serif, fontWeight: 700 }}>{oculto(v.valor)}</span></>))}
            </div>)}
            {mov.proventos.length > 0 && (<div style={box}>{secH("💵", T.gold, "Proventos recebidos", mov.proventos.length, mov.totalProventos)}
              {mov.proventos.map((p) => linha(<><span style={pill}>{p.ticker}</span><span style={{ flex: 1, minWidth: 0, fontSize: 11.5, color: T.muted }}>{p.tipo} · {dia(p.data)}</span><span style={{ fontFamily: T.serif, fontWeight: 700, color: T.gold }}>{oculto(p.valor)}</span></>))}
            </div>)}
          </>)}
        </div>
      )}
